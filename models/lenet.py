...
def evaluate_lenet5(learning_rate=0.1, n_epochs=200,
    ...
    :type datasets: iterable of data in which the first element is a tuple for training (X, y),
                    second is a tuple for validation and third is a tuple for testing.
    :param datasets: datasets used for training /testing (MNIST here)
...
def preprocess(X):
    ...
    X = numpy.apply_along_axis(lambda im: (im > 252).astype(numpy.float32), 0, X)
    ...
    X = numpy.array([resize(crop(im)) for im in X])
    X = X.reshape((-1, 784))
...
def load_labels():
    ...
    with open('../data/train_y.csv','r') as f:
        r = csv.reader(f)
        next(r,None)
        Y = numpy.array([int(val) for idx, val in r], dtype='uint8')
    return Y

def load_test():
    print("....Loading test images")
    test = numpy.fromfile('../data/test_x.bin', dtype='uint8')
...
def load_all_data():
    ...
    train_x, val_x = images[:-20000], images[-20000:]
    train_y, val_y = labels[:-20000], labels[-20000:]
    test_x = tests
...
if __name__ == '__main__':
    ...
    test = (test, numpy.array(tuple(numpy.random.randint(0, 20) for _ in xrange(20000))))
    # val = (val[0][:-10000], val[1][:-10000])
    # test = (val[0][-10000:], val[1][-10000:])

    loaded_data = (train, val, test)

    datasets = [shared_dataset(data) for data in loaded_data]
    ...
    predictions = []
    for i in xrange(20000 / 500):
        predictions.append(predictor(i))

    predictions = numpy.hstack(predictions)
    print("....Finished predicting. Writing output")
